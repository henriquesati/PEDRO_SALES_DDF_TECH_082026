...
## Versão Final (após revisão de métricas)
...
O pitch se ancora em **taxas, ratios e eficiência** — métricas transferíveis a qualquer operação, independente do ticket médio do cliente. Valores monetários existem nos dados por necessidade do modelo, mas **não são o argumento central**.

### Camada 1 — Conversão & Recuperação (métricas primárias)

| Métrica | Valor Alvo | Benchmark |
|---|---|---|
| Taxa de abandono | ~70% | Baymard Institute: 69.8% e-commerce global |
| **Taxa de recuperação geral** | **~10% dos abandonados** | Klaviyo/Salesforce: 5-15% |
| Taxa de conversão direta (sem resgate) | ~20% dos carrinhos | Sem intervenção de campanha |
| Lift de conversão com resgate vs sem | **+50% vs baseline** | Carrinhos com campanha vs sem |

### Camada 2 — Eficiência por Canal (segmentação)

| Canal | Taxa de envio→abertura | Taxa de abertura→clique | Taxa de clique→conversão | Conversão end-to-end |
|---|---|---|---|---|
| Email | ~42% | ~28% | ~15% | ~4.5% dos abandonados |
| WhatsApp | ~68% | ~35% | ~18% | ~2.5% dos abandonados |
| SMS | ~55% | ~22% | ~14% | ~1.8% dos abandonados |
| Push | ~30% | ~18% | ~12% | ~1.2% dos abandonados |

### Camada 3 — Eficiência da Segmentação RFM

| Métrica | Valor Alvo | O que demonstra |
|---|---|---|
| Taxa de recuperação Premium | ~18% | Segmento mais responsivo |
| Taxa de recuperação Regular | ~10% | Base sólida |
| Taxa de recuperação Dormant | ~6% | Menor, mas ainda lucrativo |
| Taxa de recuperação Novo | ~12% | Alta com cupom 1ª compra |
| Ratio Premium/Dormant | **3x** | Validação de que segmentar compensa |

### Camada 4 — Eficiência Operacional (ratios)

| Métrica | Valor Alvo | O que demonstra |
|---|---|---|
| ROI médio por campanha | ~45x custo | Cada R$1 investido retorna ~R$45 |
| Custo médio por conversão | < 1% do valor recuperado | Investimento desprezível vs retorno |
| % de campanhas que geram abertura | ~55% | Mais da metade engaja |
| % de campanhas que geram conversão | ~10% | 1 em 10 campanhas converte |
| Redução na taxa de expiração | ~-15% vs sem campanha | Menos carrinhos perdidos definitivamente |

### Camada 5 — Timing & Sequência

| Métrica | Valor Alvo | O que demonstra |
|---|---|---|
| % de conversões no 1º toque (lembrete_1h) | ~35% das conversões | Urgência do primeiro contato |
| % de conversões no 2º toque (lembrete_24h) | ~30% das conversões | Reforço funciona |
| % de conversões no 3º toque (desconto_48h) | ~25% das conversões | Desconto como acelerador |
| % de conversões no 4º toque (urgencia_72h) | ~10% das conversões | Última chance, menor volume |
| Tempo médio abandono→conversão | ~28h | Janela de oportunidade |

### Contexto Monetário (secundário — existe nos dados, não no pitch)

> [!NOTE]
> Os valores monetários existem nos dados para calcular ROI e custo, mas **não são apresentados como métrica de valor**. O argumento é: "a plataforma recupera ~10% dos carrinhos abandonados com ROI de 45x" — o cliente insere seu próprio ticket médio para projetar receita.

| Dado | Valor no Dataset | Propósito |
|---|---|---|
| Ticket médio geral | ~R$150-200 | Base para cálculo de ROI nos dados |
| Custo por canal | R$0.02-0.30 | Demonstrar eficiência operacional |
| Faixa de desconto | 5-15% | Demonstrar que desconto é marginal |
...
> [!IMPORTANT]
> **Abordagem de métricas**: O METRICS.md será organizado em 5 camadas (Conversão → Canal → RFM → Operacional → Timing), todas em **taxas e ratios**. Valores monetários ficam como contexto secundário nos dados, não no pitch. Confirme se essa hierarquia faz sentido.
